...
## Creating NLP chatbot with science-related questions
...
## Introduction
The aim for the document is to provide you how a Transformer based chatbot can be create with Pytorch to help user's answer their science realted questions.
...
For most of the time if projects are old ..then datasets are easily availabe from past researches and for this task we will use hugging face's dataset library.

#### Getting data
We will use Hugging Face's dataset python library to load the followinf dataset https://huggingface.co/datasets/LawalAfeez/science-dataset . This dataset consists of question and their corresponding answers. So let's see how we can load that dataset

- Note: For the sake of the documentation. We will be using a Jupiter NoteBook which is present over here https://colab.research.google.com/drive/10pJgJ8KVUJ_zK-tcsgoWEFOWdUR1jegn?usp=sharing
...
dataset = load_dataset('LawalAfeez/science-dataset')
dataset['train'].features
```

```bash
{'question': Value(dtype='string', id=None),
 'distractor3': Value(dtype='string', id=None),
 'distractor1': Value(dtype='string', id=None),
 'distractor2': Value(dtype='string', id=None),
 'correct_answer': Value(dtype='string', id=None),
 'support': Value(dtype='string', id=None),
 'embedding': Sequence(feature=Value(dtype='float32', id=None), length=-1, id=None)}
```

We can see that we have the following features which we will use later specially question, answer to train out model. Now that we have the dataset let's format the doc so that we can have data frame 
...
Let's call the conversion function

```python
df = convert_dataset(dataset['train'])
df
```

Once done our dataframe would looks something like 
...
...	...	...
11674	The enzyme pepsin plays an important role in t...	peptides
11675	What remains a constant of radioactive substan...	rate of decay
11676	Terrestrial ecosystems, also known for their d...	biomes
11677	High explosives create shock waves that exceed...	supersonic
11678	What do you call a structure composed of two o...	organ
11679 rows × 2 columns
```
...
def load_datasets():
    #Load data from hugging face for science QA
    dataset = load_dataset('LawalAfeez/science-dataset')
    df =  convert_dataset(dataset['train'])
    return df
